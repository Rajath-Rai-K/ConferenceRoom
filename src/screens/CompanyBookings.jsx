...
const CompanyBookings = () => {
    ...
    useEffect(() => {
      const fetchBookings = async () => {
          try {
          const response = await axios.get('http://localhost:8080/api/bookings/company/' + user_id);
  ...
  filteredBookings.map((booking, index) => {
    ...
    return (
      ...
      >
        ...
        {(booking.user.id === user_id) ? 
            <span className="text-center text-gray-700"><b>Me</b></span>
            :
            <span className="text-center text-gray-700">{booking.user.username}</span>
        }
